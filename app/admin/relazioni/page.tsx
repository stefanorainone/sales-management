...
export default function AdminRelazioniPage() {
  ...
  const updateWhatICanGive = (index: number, value: string) => {
    const newItems = [...(form.formData.whatICanGive || [''])];
    newItems[index] = value;
    form.updateField('whatICanGive', newItems);
  };

  const addWhatICanGive = () => {
    form.updateField('whatICanGive', [...(form.formData.whatICanGive || []), '']);
  };

  const removeWhatICanGive = (index: number) => {
    const newItems = (form.formData.whatICanGive || []).filter((_: string, i: number) => i !== index);
    form.updateField('whatICanGive', newItems);
  };

  const updateWhatICanReceive = (index: number, value: string) => {
    const newItems = [...(form.formData.whatICanReceive || [''])];
    newItems[index] = value;
    form.updateField('whatICanReceive', newItems);
  };

  const addWhatICanReceive = () => {
    form.updateField('whatICanReceive', [...(form.formData.whatICanReceive || []), '']);
  };

  const removeWhatICanReceive = (index: number) => {
    const newItems = (form.formData.whatICanReceive || []).filter((_: string, i: number) => i !== index);
    form.updateField('whatICanReceive', newItems);
  };
      ...
      >
        {editingRelation && (
          <div className="space-y-4">
            ...
            {/* Cosa Posso Dare */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">🎁 Cosa Posso Dare</label>
              <div className="space-y-2">
                {(form.formData.whatICanGive || ['']).map((item: string, index: number) => (
                  <div key={index} className="flex gap-2">
                    <Input
                      value={item}
                      onChange={(e) => updateWhatICanGive(index, e.target.value)}
                      placeholder="Es. Consulenza tecnica, networking"
                      className="flex-1"
                    />
                    {(form.formData.whatICanGive?.length || 0) > 1 && (
                      <Button variant="secondary" size="sm" onClick={() => removeWhatICanGive(index)}>
                        ✕
                      </Button>
                    )}
                  </div>
                ))}
                <Button variant="secondary" size="sm" onClick={addWhatICanGive} className="w-full">
                  + Aggiungi
                </Button>
              </div>
            </div>
            ...
            {/* Cosa Posso Ricevere */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">📥 Cosa Posso Ricevere</label>
              <div className="space-y-2">
                {(form.formData.whatICanReceive || ['']).map((item: string, index: number) => (
                  <div key={index} className="flex gap-2">
                    <Input
                      value={item}
                      onChange={(e) => updateWhatICanReceive(index, e.target.value)}
                      placeholder="Es. Referenze, opportunità di business"
                      className="flex-1"
                    />
                    {(form.formData.whatICanReceive?.length || 0) > 1 && (
                      <Button variant="secondary" size="sm" onClick={() => removeWhatICanReceive(index)}>
                        ✕
                      </Button>
                    )}
                  </div>
                ))}
                <Button variant="secondary" size="sm" onClick={addWhatICanReceive} className="w-full">
                  + Aggiungi
                </Button>
              </div>
            </div>
