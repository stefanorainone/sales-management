...
async function takeScreenshot(page, name) {
  const filename = path.join(SCREENSHOT_DIR, `local-${name}.png`);
  await page.screenshot({ path: filename, fullPage: true });
  console.log(`📸 Screenshot saved: local-${name}.png`);
}
...
async function testTaskCompletion() {
  console.log('🚀 Starting LOCAL Task Completion Test with Puppeteer\n');
  console.log(`Testing URL: ${BASE_URL}\n`);
  console.log('⚠️  Make sure you have the dev server running: npm run dev\n');
  ...
  try {
    ...
    console.log('🔍 Step 1: Checking if dev server is running...');
    try {
    ...
    console.log('  ✅ Dev server was reachable');
    ...
    console.log('  ✅ Set task duration (25 minutes)');
    console.log('  ✅ Selected outcome (Success)');
    console.log('  ✅ Tested file upload handling');
    console.log('  ✅ Clicked complete button');
    console.log('  ✅ Handled any dialogs gracefully');
