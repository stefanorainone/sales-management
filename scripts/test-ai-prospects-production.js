...
async function testAIProspectsProduction() {
  console.log('🔍 Testing AI Prospects in PRODUCTION\n');
  ...
  try {
    ...
    await page.goto(`${PROD_URL}/relazioni`, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    if (realProspectsCount === prospectsData.prospects.length) {
      ...
      console.log('💡 The AI is still generating some placeholder names like "Marco Rossi", "Laura Bianchi".\n');
    } else {
      console.log('❌ FAILURE: All prospects appear to be generic placeholders.\n');
      console.log('💡 The enhanced prompt may not be working as expected.\n');
    }
...
testAIProspectsProduction().catch(console.error);
